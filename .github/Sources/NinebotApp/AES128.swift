...
enum AES128 {
    ...
    private static func keyExpansion(key: [UInt8]) -> [[UInt8]] {
        ...
        for i in 4..<44 {
            ...
            w[i] = zip(w[i-4], temp).map { $0 ^ $1 }
        }
        var roundKeys = [[UInt8]]()
        for round in 0..<11 {
            var rk = [UInt8]()
            for i in 0..<4 { rk.append(contentsOf: w[round*4 + i]) }
            roundKeys.append(rk)
        }
        return roundKeys
    }
    ...
    private static func mixColumns(_ state: inout [UInt8]) {
        for c in 0..<4 {
            let a0 = state[c*4+0], a1 = state[c*4+1], a2 = state[c*4+2], a3 = state[c*4+3]
